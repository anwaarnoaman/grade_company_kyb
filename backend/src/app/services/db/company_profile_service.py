...
async def apply_manual_edits(db: AsyncSession,data: dict, edits: list[dict], company_id: str, actor: str):
    """
    ...
    for edit in edits:
        ...
        await log_audit(
            ...
            old_value={edit["fieldName"]: old_value},
            new_value={edit["fieldName"]: new_value},
            message=f"Manual edit on {edit['fieldName']}",
        )
...
class CompanyProfileService:
    ...
    async def save_manual_edits(
        ...
        """
        # Apply edits
        updated_data =await apply_manual_edits(db,kyb_data.get("unified_company", {}), manual_edits, company_id, actor )
